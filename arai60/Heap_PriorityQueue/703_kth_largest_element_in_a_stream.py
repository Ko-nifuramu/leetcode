...
class KthLargest:
    def __init__(self, k: int, nums: List[int]):
        self.k = k
        self.nums = nums
        self.k_largest_scores = []
        for num in nums:
            heapq.heappush(self.k_largest_scores, num)
            if len(self.k_largest_scores) > k:
                heapq.heappop(self.k_largest_scores)

    def add(self, val: int) -> int:
        heapq.heappush(self.k_largest_scores, val)
        if len(self.k_largest_scores) > self.k:
            heapq.heappop(self.k_largest_scores)
