    ...
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        ...
        def search_combinations_to_target(index, remaining_target, current_combination):
            ...
            for count in range(max_count+1):
                new_combination = current_combination + [candidates[index] for _ in range(count)]
                search_combinations_to_target(index+1, remaining_target-candidates[index]*count, new_combination)
